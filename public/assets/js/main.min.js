window.google=window.google||{};window.google.maps=window.google.maps||{};window.google.maps.Map=function(){};window.google.maps.OverlayView=class{};window.google.maps.InfoWindow=class{open(){}close(){}};window.google.maps.event={addDomListener:function(){},addListener:function(){},trigger:function(){},};window.markerClusterer=window.markerClusterer||{};window.markerClusterer.MarkerClusterer=function(){};(function(){"use strict";gsap.registerPlugin(ScrollTrigger);window.App={};App.config={cursorFollower:{enabled:!0,disableBreakpoint:'992',},}
...
function initialReveal(){revealAnimFallback()
RevealAnim.init()
initComponents()}
document.addEventListener('DOMContentLoaded',revealAnimFallback);
function revealAnimFallback(){setTimeout(function(){document.querySelectorAll('[data-anim]:not(.is-in-view), [data-anim-child]:not(.is-in-view)').forEach(function(el){el.classList.add('is-in-view')});document.querySelectorAll('[data-anim-wrap]:not(.animated)').forEach(function(el){el.classList.add('animated')})},2000)}
function initComponents(){sectionSlider();testimonialsSlider_1();marquee();menuEvents();headerSticky();dbSidebarToggle();Header.init();Tabs.init();Accordion.init();lazyLoading();parallaxInit();mapCard();galleryInit();Cursor.init();heroSlider7();heroSlider9();heroSlider10();Select.init(".js-select");priceRangeSliderInit();requestForm();splitText();parallaxIt();hero1Reveal();toTopButton();tabsSlider();hero5Reveal();testimonialsSlider1();testimonialsSlider_2();lineChart();dropdown();countChange();if(typeof Events!=="undefined"&&Events.init){Events.init()}
pinOnScroll();if(typeof Calendar!=="undefined"&&Calendar.init){Calendar.init();calendarInteraction()}
selectControl();liveSearch()}
function liveSearch(){const targets=document.querySelectorAll('.js-liverSearch')
if(!targets)return
const data=[{icon:"icon-pin",title:"Phuket",text:"Thailand, Asia"},{icon:"icon-price-tag",title:"London Day Trips",text:"England"},{icon:"icon-flag",title:"Europe",text:"Country"},{image:"img/misc/icon.png",title:"Centipede Tour - Guided Arizona Desert Tour by ATV",text:"Country"},{icon:"icon-pin",title:"Istanbul",text:"Turkey"},{icon:"icon-pin",title:"Berlin",text:"Germany, Europe"},{icon:"icon-pin",title:"London",text:"England, Europe"},]
targets.forEach(el=>{const search=el.querySelector('.js-search')
const results=el.querySelector('.js-results')
let searchTerm=''
results.querySelectorAll('.js-search-option').forEach(option=>{const title=option.querySelector('.js-search-option-target').innerHTML
option.addEventListener('click',()=>search.value=title)})
search.addEventListener('input',(event)=>{searchTerm=event.target.value.toLowerCase()
showList(searchTerm,results)
results.querySelectorAll('.js-search-option').forEach(option=>{const title=option.querySelector('.js-search-option-target').innerHTML
option.addEventListener('click',()=>search.value=title)})})})
const showList=(searchTerm,resultsEl)=>{resultsEl.innerHTML='';data.filter((item)=>item.title.toLowerCase().includes(searchTerm)).forEach((e)=>{const div=document.createElement('div')
if(e.image){div.innerHTML=`
...
function mapCard(){const targets=document.querySelectorAll('.js-mapPlaces')
targets.forEach((target)=>{const cards=target.querySelectorAll('[data-map-card]')
const buttons=target.querySelectorAll(`[data-map-place]`)
cards.forEach((el)=>{const attrVal=el.getAttribute('data-map-card')
const button=target.querySelector(`[data-map-place="${attrVal}"]`)
el.addEventListener('click',(e)=>{cards.forEach((el)=>el.classList.remove('isCardActive'))
buttons.forEach((el)=>el.classList.remove('isActive'))
if(!el.classList.contains('isCardActive')){button.classList.toggle('isActive')
...
noUiSlider.create(slider,{start:[20,70000],step:1,connect:!0,range:{'min':0,'max':5000},format:{to:function(value){return"$"+value.toFixed(0)},from:function(value){return value}}})
...
new ScrollMagic.Scene({duration:pageContentHeight-1600,}).setClassToggle(button,'is-hidden').addTo(App.SMcontroller)
...
swiper.on('slideChange',function(){console.log(swiper.realIndex)
images.style.transform=`translateX(${swiper.realIndex * -100}%)`})}
function lineChart(){const ctx=document.getElementById('lineChart');if(!ctx)return;const myChart=new Chart(ctx,{type:'line',data:{labels:['Jan','Feb','Marc','April','May','Jun','July','Agust','Sept','Oct','Now','Dec',],datasets:[{label:'#',data:[148,100,205,110,165,145,180,156,148,220,180,245],tension:0.4,backgroundColor:'#336CFB',borderColor:'#336CFB',borderWidth:2,}]},options:{responsive:!0,plugins:{legend:{display:!1,},},scales:{y:{min:0,max:300,ticks:{stepSize:50}}},},})}
...
return{init:init,leaveHandler:leaveHandler,update:update,clear:clear,hide:hide,show:show,}})();class HTMLMapMarker extends google.maps.OverlayView{constructor(args){super();this.latlng=args.latlng
this.html=args.html
...
appendDivToOverlay(){const panes=this.getPanes();panes.overlayMouseTarget.appendChild(this.div)}
positionDiv(){const point=this.getProjection().fromLatLngToDivPixel(this.latlng);if(point){this.div.style.left=`${point.x}px`;this.div.style.top=`${point.y}px`}}
...
getVisible(){return this.latlng}
getPosition(){return new google.maps.LatLng(this.latlng)}
getDraggable(){return!1}}
function initMap(){if(!document.querySelector('.js-map'))return
const map=new google.maps.Map(document.querySelector('.js-map'),{zoom:12,center:{lat:40.69,lng:-73.88}})
const locations=[{lat:40.800610,lng:-74.035242},{lat:40.730610,lng:-73.935242},{lat:40.740610,lng:-73.825242},{lat:40.700610,lng:-73.885242},{lat:40.670610,lng:-73.785242},{lat:40.680610,lng:-73.905242},]
const contentString=`
  ...
  `;const markers=locations.map((location)=>{const marker=new HTMLMapMarker({latlng:location,map:map,html:`
...
marker.addListener("click",()=>{setTimeout(()=>{infowindow.open({anchor:marker,map,shouldFocus:!1,})},50)})
return marker})
new markerClusterer.MarkerClusterer({map,markers:markers})}
function initMapTourPages(){if(!document.querySelector('.js-map-tour'))return
const map=new google.maps.Map(document.querySelector('.js-map-tour'),{zoom:10,center:{lat:40.8,lng:-74.02}})
const locations=[{lat:40.800610,lng:-74.035242},{lat:41.000610,lng:-74.135242},{lat:40.700610,lng:-73.835242},]
const contentString=`
  ...
  `;const markers=locations.map((location,index)=>{const marker=new HTMLMapMarker({latlng:location,map:map,html:`
...
return marker})
new markerClusterer.MarkerClusterer({map,markers:markers})}
function initMapSingle(){if(!document.querySelector('.js-map-single'))return
new google.maps.Map(document.querySelector('.js-map-single'),{zoom:12,center:{lat:40.8,lng:-74.02}})}
const Events=(function(){function init(){const targets=document.querySelectorAll("[data-x-click]")
if(!targets)return
...
if(!targets)return
targets.forEach(el=>{if(el.querySelector('.-is-visible')){el.querySelector('.-is-visible').classList.remove('-is-visible')}})}}
const Calendar=(function(){const _today=new Date()
const startYear=_today.getFullYear()
const startMonth=_today.getMonth()+1
const monthRange=12
const weekDaysOrder=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"]
let calendarEl
function updateVars(){calendarEl=document.querySelectorAll('.js-calendar-el')
if(!calendarEl.length)return}
const getAllDaysInMonth=(month,year)=>{let initialMonthArray=Array.from({length:new Date(year,month,0).getDate()},(_,i)=>{return{weekDay:weekDaysOrder[new Date(year,month-1,i+1).getDay()],dayNum:i+1,}})
...
testDate=Array.from({length:new Date(testDate).getDate()},(_,i)=>{return{weekDay:weekDaysOrder[new Date(new Date(testDate).getFullYear(),new Date(testDate).getMonth(),i+1).getDay()],dayNum:i+1,}}).slice(-daysInBeginning)
if(daysInBeginning==0){testDate=[]}
const months=["january","february","march","april","may","june","july","august","september","october","november","december"]
let monthName=months[new Date(year,month,0).getUTCMonth()]
return{monthName,initialMonthArray,firstDates:testDate}}
function getFullYearDates(startDate,countOfMonthToTake){const date=startDate.split('/')
let allYearMonths=[]
for(let i=date[0];i<parseInt(date[0])+parseInt(countOfMonthToTake);i++){allYearMonths.push(getAllDaysInMonth(i,date[1]))}
return allYearMonths}
function init(){updateVars()
if(!calendarEl.length)return
calendarEl.forEach(calendarElement=>{let calendarGrid=calendarElement.querySelector('.js-calendar-el-calendar')
let allYearMonths=getFullYearDates(`${startMonth}/${startYear}`,monthRange)
let globalIndex=0
function globalIndexUp(){globalIndex=globalIndex+1
return globalIndex}
calendarGrid.innerHTML+=`
        <div class="elCalendar__slider js-calendar-slider">
          <div class="swiper-wrapper">
            ${allYearMonths.map((month, i) => {
              return `<div class="swiper-slide"><div class="capitalize text-18 fw-500 text-center mb-20">${month.monthName}${startMonth+i>12?startYear+1:startYear}</div><div class="elCalendar__month"><div class="elCalendar__header">${weekDaysOrder.map(el=>`<div class="elCalendar__header__sell">${el}</div>`).join('')}</div><div class="elCalendar__body">${month.firstDates.map(el=>`
                        <div
...
function calendarSlider(container){const attributeCheck=container.getAttribute('data-slider-cols-2');let breakpoints=!1;if(attributeCheck){breakpoints={991:{slidesPerView:2},}}
new Swiper(container.querySelector('.js-calendar-slider'),{speed:600,autoHeight:!0,spaceBetween:30,slidesPerView:1,breakpoints:breakpoints,navigation:{prevEl:'.js-calendar-slider-prev',nextEl:'.js-calendar-slider-next',},})}
return{init:init,}})();function calendarInteraction(){const target=document.querySelectorAll('.js-calendar')
if(!target)return
target.forEach(elTarget=>{const gridCells=elTarget.querySelectorAll('.elCalendar__body > *')
const firstDate=elTarget.querySelector('.js-first-date')
const lastDate=elTarget.querySelector('.js-last-date')
let completeState=!1
let firstItem=!1
let lastItem=!1
gridCells.forEach((el,i)=>{el.addEventListener('click',()=>{el.classList.add('-is-active')
if(firstItem&&getIndex(firstItem)>getIndex(el)){lastItem=firstItem
firstItem=el}
if(firstItem&&!lastItem){lastItem=el}
if(!firstItem){firstItem=el}
if(completeState){firstItem=!1
lastItem=!1
const array=elTarget.querySelectorAll('.-is-active')
array.forEach(el2=>{el2.classList.remove('-is-active')})
const array2=elTarget.querySelectorAll('.-is-in-path')
array2.forEach(el2=>{el2.classList.remove('-is-in-path')})
completeState=!1}else if(firstItem&&lastItem){const iterationCount=Math.abs(getIndex(firstItem)-getIndex(lastItem))
for(let l=1;l<iterationCount;l++){const item=elTarget.querySelector(`[data-index="${ getIndex(firstItem) + l }"]`)
...
completeState=!0}})})})
function getIndex(element){return parseInt(element.getAttribute('data-index'))}}})()